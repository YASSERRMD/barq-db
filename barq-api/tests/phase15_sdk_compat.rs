...
#[tokio::test]
async fn test_sdk_public_clients_remain_compatible() {
...

    {
        ...
        client
            .create_collection("sdk-rust-compat", 2, DistanceMetric::Cosine, None, None)
            .await
            .expect("rust create collection");

        let collection = client.collection("sdk-rust-compat");
        collection
            .insert(
                barq_core::DocumentId::U64(100),
                vec![1.0, 0.0],
                Some(serde_json::json!({"sdk": "rust", "mode": "compat"})),
            )
            .await
            .expect("rust insert");
    ...
    run_command(
        "python compat client",
        &workspace_root.join("barq-sdk-python"),
        &common_env,
        "python3",
        ["-m", "unittest", "discover", "-s", "tests", "-p", "test_compat_client.py"],
    )
    .await;
    ...
    run_command(
        ...
        &common_env,
        "go",
        ["test", "./...", "-run", "TestCompatClientCreateInsertSearch", "-count=1"],
    )
    .await;
